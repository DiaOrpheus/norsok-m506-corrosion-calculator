...
def pHCalculator(ITemp, IPressure, ICO2Pressure, IBicarb, IIonicStrength, CalcOfpH):
    ...
    for pHCalcNo in range(1,CalcOfpH + 1):
...
def Kt(temp):
    ...
    if temp < temp_table[0] or temp > temp_table[-1]:
        raise ValueError(
            f"Temperature {temp}°C outside Kt table range (5-150°C)"
        )
    ...
    for i,temp_i in enumerate(temp_table):
        if temp < temp_i:
            ...
            return value_lower + \
                (temp-temp_lower)/(temp_upper-temp_lower) * \
                (value_upper-value_lower)

def Cal_Norsok(CO2fraction, Pressure, Temp, v_sg, v_sl , mass_g, mass_l,vol_g,vol_l,holdup,vis_g,vis_l,roughness,diameter,\
